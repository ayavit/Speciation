...
int main(int argc, char* argv[])
{
	...
	int NCH = 1000;//1000;   //size of population 
	int NGEN = 60000;   // number of generations to run
	double MUT_PROB = 0.1;//0.1;   // probability of new mutation in a new individual
	...
	for (gen=0; gen<NGEN; gen++){
		...
		for (i=0; i<NCH; i++){
			...
			do {
	            ...
	            // free and create list of offspring mutations
	            // if (new_record.n>0)
				free (new_record.list);
			    free (new_record.list_hash);
			    free (new_record.two_copies);
	...
	//				mut_list[i].list = (unsigned int *) realloc (mut_list[i].list, (mut_list[i].nmax+20)*sizeof (unsigned int)) ;
		...
		if (gen % 10 == 9){
            ...
            if (gen % 1000 == 9){
	            ...
	            sprintf(fffname, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_failures.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            fff = fopen(fffname, "w");

	            // open file to print number of successes out of 1000 trials for couples of individuals in a list of distances
	            // (1, 10, 50, 100, 500)
                sprintf(ffname, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_successes.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            ff = fopen(ffname, "w");
			...
			if ((gen % 10 ==9 && gen < 118000)){// || (gen % 10 ==9 && gen > 8000)){
		...
		if (gen % 1000 == 999){ 
			...
			sprintf(fname, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_just_0_1.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<5; t++){
				for (r=0; r<NCH; r++){
			...
			sprintf(fname, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_causative.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<5; t++){
				for (r=0; r<NCH; r++){
					int loc1 = (r + try_dists[t]) % NCH;
			...
			//sprintf(fname, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_times.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			//f = fopen(fname, "w");
			...
			// name of the file
			sprintf(fname_all_mutations, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_all_mutations.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			...
			// name of the file
			sprintf(fname_all_mutations, "sympatric_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_two_copies.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
