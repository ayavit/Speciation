...
// vector of first differences between checked parents
int first_differences[5] = {0,0,0,0,0};
...
// function that receives pointers to two mutation structures and returns 1 if the mating of the two was successful and 0 if not.
//In addition, it returns the (first) mutations that caused inviability.
int is_success (mut_st *mp1_point, mut_st *mp2_point, unsigned int *cause1, unsigned int *cause2, unsigned int *cause3, unsigned int *cause4, unsigned int *cause5)
{

    // pointers to the first five causative mutations
	*cause1 = 0;
	*cause2 = 0;
	*cause3 = 0;
	*cause4 = 0;
	*cause5 = 0;
	...
	int num_of_differences = 0;   // counter for the number of differences found between parents

	while (num_of_differences < 5){
		...
		// parent's list, record the mutation in index i2 and move index i2 forward.
		if ((i1 == mp1.n)||(i2 < mp2.n && mp1.list[i1]>mp2.list[i2])){
	        first_differences[num_of_differences] = mp2.list[i2];
		   	num_of_differences++;
			i2++;
	    }
	        ...
	        // parent's list, record the mutation in index i1 and move the index i1 forward.
	        if ((i2 == mp2.n)||(i1 < mp1.n && mp1.list[i1]<mp2.list[i2]) ){
	            first_differences[num_of_differences] = mp1.list[i1];
		    	num_of_differences++;
				i1++;
	        }
	            ...
	            // If we found the same mutation in both parents, move indices forward.
	            if (i1 < mp1.n && i2 < mp2.n &&mp1.list[i1]==mp2.list[i2]){
	                first_differences[num_of_differences] = mp1.list[i1];
		            i1++;
					i2++;
    ...
    // If there is an incompitability
    if (num_of_differences >= 5){ 

        // save the mutation that caused inviability
		(*cause1) = first_differences[0];
		(*cause2) = first_differences[1];
		(*cause3) = first_differences[2];
		(*cause4) = first_differences[3];
		(*cause5) = first_differences[4];
...
int main(int argc, char* argv[])
{
	...
	int NCH = 10000;   //size of population 
	int NGEN = 6000;   // number of generations to run
	double MUT_PROB = 0.1;   // probability of new mutation in a new individual
    double theta = 0.2;   // exponential function parameter. If theta=0 then the choice of parents does not depend on the distance between them.
	
	unsigned int seed_for_rand = 0;   // seed for randomization
    
	unsigned int inv_cause1 = 0, inv_cause2 = 0, inv_cause3 = 0, inv_cause4 = 0, inv_cause5 = 0;   // the (first) mutations that cause inviability of a couple's offspring
	...
	char mut_string3 [33];   // string to convert the first causative mutation number into
	char mut_string4 [33];   // string to convert the second causative mutation number into
	char mut_string5 [33];   // string to convert the second causative mutation number into
	...
	int try_dists[7] = {1,10,50,100,500,1000,5000};
	...
	for (gen=0; gen<NGEN; gen++){
		...
		for (i=0; i<NCH; i++){
			...
			do {
				...
				// if theta=0, choose both parents randomly
				if (theta==0){
					tmp1 = (((unsigned long)rand()*0x10000) + rand()) % NCH;
					tmp2 = (((unsigned long)rand()*0x10000) + rand()) % NCH;
				}
								
				// if theta>0, choose parents using exponential decay function
				else
				    do {
					    rn1 = ((double) rand())/RAND_MAX+0.00000001, rn2 = ((double) rand())/RAND_MAX+0.00000001;
					    tmp1 = (int) (-log(rn1)/theta), tmp2 = (int) (-log(rn2)/theta);
				    } while(tmp1>=NCH/2 || tmp2>=NCH/2);
				...
				p2 = (i + sgn2 * tmp2 + NCH) % NCH ;
 				...
 				// mutation structures of parents
				mp1 = mut_list[p1];
				mp2 = mut_list[p2];
	            ...
	            // Each mutation from parent 2 is passed to the offspring with probability 0.5 (if the other parent doesn't have it).
	            for (int m2 = 0; m2<mp2.n ; m2++){
		            p1_m2[m2] = rand() >= RAND_MAX/2;
	            } 

	            // Each mutation from parent 1 is passed to the offspring with probability 0.5 (if the other parent doesn't have it).
	            ...
	            for (int m2 = 0; m2<mp2.n ; m2++) new_record.n++;
	            for (int m1 = 0; m1<mp1.n ; m1++) new_record.n++;
	            ...
	            int i1 = 0;   // index in parent 1's mutation list
	            int i2 = 0;   // index in parent 2's mutation list
				int differences = 0;    // counter for the number of differences between parents
	            ...
	            while (true){
		            ...
		            if ((i1 == mp1.n)||(i2 < mp2.n && mp1.list[i1]>mp2.list[i2])){
		                ...
		                if ((i2 == mp2.n)||(i1 < mp1.n && mp1.list[i1]<mp2.list[i2]) ){
		                    ...
		                    // If we found the same mutation in both parents, record the mutation and move indices forward.
		                    // Also, subtract 1 from the offspring's count of different mutations. 
		                    if (i1 < mp1.n && i2 < mp2.n &&mp1.list[i1]==mp2.list[i2]){
			                    unsigned int mm1 = mp1.list[i1];
					            new_record.list[temp] = mm1;
					            new_record.list_hash[temp] = mp1.list_hash[i1];
		...
		if (gen % 10 == 9){
            ...
            if (gen % 1000 == 9){
	            ...
	            sprintf(fffname, "with_selection_haploid_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_failures.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            fff = fopen(fffname, "w");

	            // open file to print number of successes out of 10000 trials for couples of individuals in a list of distances
	            // (1, 10, 50, 100, 500, 1000, 5000)
                sprintf(ffname, "with_selection_haploid_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_successes.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            ff = fopen(ffname, "w");
			...
			for (t=0; t<7; t++){
				...
				for (r=0; r<10000; r++){
					...
					alive = true;
					success+= is_success (mut_list+loc, mut_list+loc1, &inv_cause1, &inv_cause2, &inv_cause3, &inv_cause4, &inv_cause5);
				}

				// print number of successes out of 10000 trials for couples of individuals in a list of distances
			    // (1, 10, 50, 100, 500, 1000, 5000) to a file
		...
		if (gen % 1000 == 999){ 
			...
			sprintf(fname, "with_selection_haploid_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_just_0_1.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<7; t++){
				for (r=0; r<NCH; r++){
					...
					if ((is_success (mut_list+r, mut_list+loc1, &inv_cause1, &inv_cause2, &inv_cause3, &inv_cause4, &inv_cause5) == 1)){
						fputc ('1', f);
						fputc (' ', f);
					}
					else{
			...
			sprintf(fname, "with_selection_haploid_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_causative.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<7; t++){
				for (r=0; r<NCH; r++){
					int loc1 = (r + try_dists[t]) % NCH;
					...
					if ((is_success (mut_list+r, mut_list+loc1, &inv_cause1, &inv_cause2, &inv_cause3, &inv_cause4, &inv_cause5) != 1)){
						...
						Itoa (inv_cause3, mut_string3, 10);
						Itoa (inv_cause4, mut_string4, 10);
						Itoa (inv_cause5, mut_string5, 10);
            ...
            // print the list of all the mutations of every 10th individual
			
			// name of the file
			sprintf(fname_all_mutations, "with_selection_haploid_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_all_mutations.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
