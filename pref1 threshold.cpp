...
//	t2 = clock();
	...
	int num_of_differences = 0;   // counter for the number of differences found between offspring's chromosomes
		...
		for (int m1 = 0; m1<new_record.n ; m1++){
		    ...
		    if (new_record.two_copies[m1] == false){
				...
				if (current_difference <= 3){
			        if (current_difference = 3){
                        (*cause3) = new_record.list[m1];
					}
					else{
						if (current_difference = 2){
                            (*cause2) = new_record.list[m1];
						}
						else{
				    ...
				    if (current_difference = 4){
                            (*cause4) = new_record.list[m1];
					}
					else{
                        (*cause5) = new_record.list[m1];
					}
				}
...
int main(int argc, char* argv[])
{
	...
	unsigned int seed_for_rand = 10;   // seed for randomization
...
//	clock_t t1, t1_tot = 0, t2, t2_tot = 0, t3, t3_tot = 0, t4, t4_tot = 0;
	...
	int gen, i, t, r;   // loop indices
	 ...
	 // open file to print rn1, rn2, tmp1, tmp2, p1, p2, i, sgn1 and sgn2
	 //sprintf(ffffname, "rn1_rn2_tmp1_tmp2_p1_p2_i_26.txt"); 
	 //ffff = fopen(ffffname, "w");
	...
	for (gen=0; gen<NGEN; gen++){
		...
		for (i=0; i<NCH; i++){
			...
			do {
                ...
                //if (gen >= 20  &&  gen <= 30 ){
			    //    // print rn1, rn2, tmp1, tmp2, p1, p2, i, sgn1 and sgn2 to a file
			    //    fprintf (ffff, "%f %f %d %d %d %d %d %d %d\n", rn1, rn2, tmp1, tmp2, p1, p2, i, sgn1, sgn2);
				///}
	            ...
	            int i1 = 0;   // index in parent 1's mutation list
	            int i2 = 0;   // index in parent 2's mutation list
				int differences = 0;    // counter for the number of differences between the offspring's chromosomes
	...
	//				mut_list[i].list = (unsigned int *) realloc (mut_list[i].list, (mut_list[i].nmax+20)*sizeof (unsigned int)) ;
		...
		if (gen % 10 == 9){
            ...
            if (gen % 1000 == 9){
	            ...
	            sprintf(fffname, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_failures.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            fff = fopen(fffname, "w");
                ...
                sprintf(ffname, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_number_of_successes.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
	            ff = fopen(ffname, "w");
		...
		if (gen % 1000 == 999){ 
			...
			sprintf(fname, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_just_0_1.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<7; t++){
				for (r=0; r<NCH; r++){
			...
			sprintf(fname, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_causative.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			f = fopen(fname, "w");
			for (t=0; t<7; t++){
				for (r=0; r<NCH; r++){
					int loc1 = (r + try_dists[t]) % NCH;
			...
			//sprintf(fname, "with_selection_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_times.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			//f = fopen(fname, "w");
			...
			// name of the file
			sprintf(fname_all_mutations, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_all_mutations.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
			...
			// name of the file
			sprintf(fname_all_mutations, "with_selection_threshold_NCH%dCONT_PROB%fNGEN%dMUT_PROB%ftheta%fseed_for_rand%dgen%d_two_copies.txt",NCH, CONT_PROB, NGEN, MUT_PROB, theta, seed_for_rand, gen); 
